...
deno task test        # run pipeline + import-cycle tests
...
- **Auto-tune** — `autoTuneStrengths` in [blitzoom-utils.js](docs/blitzoom-utils.js) optimizes strengths/alpha/quant by maximizing **spread × clumpiness × group-purity** at an adaptive grid level. Purity averages over ALL cached categorical groups (2-50 distinct values), not just the dominant weighted one — cross-group mixing costs score under any config. An optional `opts.fnrPenalty` hook exists but is not enabled by shipped callers (measured ineffective; see the research doc). Dual-pass search: runs preset → descent → refine at both α=0 (property-only) and α=0.5 (moderate topology) to discover strength configurations that synergize with topology. α capped at 0.75 for property datasets to prevent CV inflation. After strengths, `autoTuneBearings` runs closed-form trace maximization to optimize per-group rotations (including solo-strength configs — there is no user-set-count guard; the strength floor makes every group effective). Async with portable yield, memoized, supports `AbortSignal` and timeout. Viewer auto-runs on fresh loads without preset settings via `_autoTuneFresh()`. For datasets >50K nodes, tunes on a 50K subsample (strength ratios transfer). Beats ForceAtlas2/t-SNE/UMAP on 3 of 5 property datasets at 1000-70000× faster. See [agent_docs/ARCHITECTURE-auto-tune.md](agent_docs/ARCHITECTURE-auto-tune.md).
...
- **Projection seeds** — per-group PRNG seed overrides for the Gaussian projection matrices: sparse map `projSeeds: {groupName: seed}` (constructor option, `proj-seeds="group:37403"` attribute on `<bz-graph>`, `setProjSeed(group, seed)` / `bulkSetProjSeeds(obj)` on the canvas; the latter re-projects changed groups without blending). Auto-tune scores 50 candidate seeds for the dominant single-token categorical group (2-20 distinct values, no numeric bins) by analytic collision mass and applies the winner — measured −45% to −95% false-neighbor rate on affected datasets ([agent_docs/RESEARCH-false-neighbor-validation.md](agent_docs/RESEARCH-false-neighbor-validation.md)). Serialized in the URL hash as `sd=` (positional absolute seeds, emitted only when overridden) and in dataset presets as `settings.projSeeds`. Worker loads always project with default seeds; overrides apply post-load via `bulkSetProjSeeds` re-projection.
...
- **Density heatmap normalization** — the kernel splat density grid (`_densityW`) is normalized against the **95th percentile** of non-zero cell weights (`_densityNorm`), not the absolute max. Cells at or above p95 saturate to full color; the rest render at intensity proportional to `weight / p95`. The percentile adapts to distribution shape: at fine aggregation levels with sharp peaks, p95 << max so the densest cores still pop; at coarse levels with few uniformly-spread supernodes, p95 ≈ max so saturation stays restricted to the genuine top ~5% instead of blanket-coloring most of the cluster footprint. Implemented as a 256-bin histogram pass alongside the existing maxW scan, walked from the top to find the bin covering 5% of non-zero cells. Lerped over ~500ms to avoid jarring jumps on level/zoom change. Cache key includes `_blendGen` so incremental adds invalidate the cached normalization base.
- **Zoom target highlight** — during scroll-wheel zoom-in, the zoom target (`zoomTargetId`) gets the same highlight treatment as hovered nodes (full label, glow, full opacity). Target selection prefers `hitTest` (cursor over circle/label) with `_nearestItem` fallback. On level change, tracks the dominant member of the old supernode to the new level. Cleared by `_zoomTargetTimer` (a debounced 300ms timer reset on every wheel tick) when wheel events stop arriving — the DOM has no `wheelend` event. Also cleared by an explicit zoom-out wheel tick.
...
- **Fast mode** — interactive drag on large datasets (> `BlitZoomCanvas.FAST_NODE_THRESHOLD = 50K` nodes) uses adaptive blend passes (0-2, budget system with ceiling lock) and spatial subsampling (16×16 grid from gx/gy, degree-weighted, ~20-50K sample). Edges suppressed via `_skipEdgeBuild` flag (stays true for entire drag session). Full 5-pass blend + layout + edge build on release. Below the threshold, drag always uses full blend. Exposed as **`view.fastRebuild()` / `view.endFastRebuild()`** public methods on `BlitZoomCanvas` so the viewer, `<bz-graph>` panels, and standalone `<bz-compass>`/`<bz-controls>` companion bindings all use the same code path. Sample staleness is detected via `_sampleNodesN` (the node count when the sample was built) — if `addNodes`/`removeNodes` changes the count, the next `fastRebuild` rebuilds the sample. The two methods serialize on `_fastRebuildPromise` so a `change` event can't run cleanup mid-drag while a pending rAF still has work to do.
...
- **URL hash state** — compact positional format. View: `d`, `l`, `z`, `x`, `y`, `bl`, `s`. Settings (all-or-nothing block): `st=5,0,8` (strengths, 3 decimals), `b=28.6,0,0` (bearings in degrees, 2 decimals), `a=0.5` (alpha, 3 decimals), `cb=1` (colorBy group index, -1=auto), `lp=0,2` (label prop indices), `sd=2001,37403` (projection seeds, absolute, emitted only when overridden; absent = defaults). Positional arrays indexed by `groupNames` order; lengths sanity-checked on restore. Matches curated datasets (`d=id`) and URL-loaded datasets (`edges=url`). `replaceState` on render.
...
- Heatmap density normalization base (`_densityNorm`, the 95th percentile of non-zero cell weights) is cached per `(instance | _blendGen | level | renderZoom | sizeBy | sizeLog | W | H)`, lerped on change with a 200ms time constant — stable across pan, smooth across zoom and strength changes, snaps on level change and instance change.
- `view.fastRebuild()` and `view.endFastRebuild()` are serialized via `_fastRebuildPromise` so they can't interleave on `view.nodes` / `_sampleNodes` / `_skipEdgeBuild`. Companions cancel pending rAFs on `change` events to prevent a queued `fastRebuild` from running after `endFastRebuild` has cleaned up. Without both layers, drag-end leaves fast mode "stuck on" with the subsample swap re-engaged.
...
- Initial level picked by `pickInitialLevel` in `_finalizeLoad` AFTER `gx/gy` is populated by blend + quantize. Among levels with 25-400 distinct cells and ≥30% of nodes in multi-member cells, picks the one closest (log space) to a size-scaled target (4·√N, clamped [40, 300]); if the counts jump straight over the band, backs off to the level before the overflow. Dataset preset `initialLevel` and URL hash `l=` still override.
...
- `projSeeds` overrides flow to every matrix build and σ derivation: canvas constructor, `bootstrapEmptyGraph`, `fullRebuild`, `computeProjections`/`computeProjectionsGPU`, and `normQuantize`/`radialQuantize` (norm-mode σ must derive from the same seeds as the matrices, or positions shift). The worker load path is seed-unaware by design — fresh loads reset `projSeeds`, and overrides re-apply post-load via `bulkSetProjSeeds` re-projection of only the changed groups.
